...
def get_data_loader(args):
	...
	if args.dataset == 'imagenet' or args.dataset == 'imagenette':
		...
		if args.dataset == 'cifar':
			...
			val_dataset = datasets.CIFAR100(root=DATA_DIR, train=False, download=True, transform=ds_transform)
		elif args.dataset == 'svhn':
			val_dataset = datasets.SVHN(root=DATA_DIR, split='test', download=True, transform=ds_transform)
		elif args.dataset == 'gtsrb':
			val_dataset = datasets.GTSRB(root=DATA_DIR, split='test', download=True, transform=ds_transform)
		elif args.dataset == 'flowers102':
			val_dataset = datasets.Flowers102(root=DATA_DIR, split='test', download=True, transform=ds_transform)
		else:
...
def build_pcure_model(args):
	...
	elif args.dataset == 'cifar': num_classes = 10
	...
	elif args.dataset == 'flowers102': num_classes = 102

	else:
		raise NotImplementedError("error, not supportted datasets")
	...
	if 'bagnet' in MODEL_NAME: #bagnet
		...
		# change the classes
		srf.fc = nn.Linear(in_features=srf.fc.in_features, out_features=num_classes)
		if split_point<0: # 
...
def build_undefended_model(args):
	# attacked_model modify!!!!!!!!!!!!!!!!!!!!
	MODEL_DIR=os.path.join('.',args.model_dir)
	MODEL_NAME = args.attacked_model.split('_')[0]
	if 'bagnet' in MODEL_NAME: #bagnet
		model_func = BAGNET_FUNC[MODEL_NAME]
	...
	# load model
	checkpoint_name = args.attacked_model + '.pth.tar'
	try:
		checkpoint_path = os.path.join(MODEL_DIR,checkpoint_name) 
