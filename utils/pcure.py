...
class SecurePooling(torch.nn.Module):
	...
	def certify(self,x,y):
		#add
		one_mask_logits,unmasked_logits = self._get_one_mask_logits(x,return_unmasked=True)
		pred = torch.argmax(unmasked_logits,dim=1) #[B,1,1]
		pred = pred.flatten()
		#add
		...
		return results, prediction_map, pred
		# new
		# one_mask_logits,unmasked_logits = self._get_one_mask_logits(x,return_unmasked=True)
		# pred = torch.argmax(unmasked_logits,dim=1) #[B,1,1]
		# pred = pred.flatten()

		# prediction_map = -torch.ones([num_img,num_mask,num_mask,num_mask,num_mask],dtype=torch.uint8,device = x.device)
		# for i,mask_1 in enumerate(mask_list):
		# 	for j in range(i,num_mask):
		# 		mask_2 = mask_list[j]
		# 		for k in range(j, num_mask):
		# 			mask_3 = mask_list[k]
		# 			for l in range(k, num_mask):
		# 				mask_4 = mask_list[l]
		# 				# masked_output = self.lrf(mask_1*mask_2*mask_3*mask_4*x)
		# 				masked_output =torch.sum(mask_1*mask_2*mask_3*mask_4*x, dim=(2, 3), keepdim=True)
		# 				_, masked_pred = masked_output.max(1)
		# 				prediction_map[:,i,j,k,l] = masked_pred
		# for img_i in range(num_img):
		# 	pred_map = prediction_map[img_i]
		#
		# 	pred_map = torch.where(pred_map == -1, y[img_i], pred_map)
		#
		# 	# pred_map = pred_map + pred_map.T - torch.diag(torch.diag(pred_map))
		# 	results[img_i] = torch.all(pred_map==y[img_i])
		# print("4 mask")
...
class SecureLayer(SecurePooling):
	...
	def certify(self,x,y):
		...
		prediction_map = -torch.ones([num_img,num_mask,num_mask,num_mask,num_mask],dtype=int,device = x.device)
		for i,mask_1 in enumerate(mask_list):
			for j in range(i,num_mask):
				mask_2 = mask_list[j]
				for k in range(j, num_mask):
